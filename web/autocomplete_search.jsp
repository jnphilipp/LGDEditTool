...
try {
	...
	if ( query.contains(":") || query.length() > 2 ) {
		...
		if ( query.startsWith("k:") ) {
			...
			Object[][] a = database.execute("SELECT kvm.v, COALESCE((SELECT SUM(usage_count) FROM lgd_stat_tags_kv WHERE v=kvm.v AND k IN (SELECT k FROM lgd_map_resource_kv WHERE v=kvm.v)), 0) AS usage_count FROM lgd_map_resource_kv AS kvm WHERE (user_id = '" + (User.getInstance().getView().equals(Functions.MAIN_BRANCH) ? "main" : User.getInstance().getUsername()) + "' AND property != '' AND object != '') OR (user_id = 'main' AND (kvm.k, kvm.v) NOT IN (SELECT k, v FROM lgd_map_resource_kv WHERE user_id = '" + (User.getInstance().getView().equals(Functions.MAIN_BRANCH) ? "main" : User.getInstance().getUsername()) + "')) AND UPPER(kvm.v) LIKE UPPER('" + query.substring(2) + "') GROUP BY v ORDER BY v");
