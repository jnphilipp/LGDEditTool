...
public class TemplatesEditHistory {
	...
	private static String kMapping(String search) throws Exception {
		DatabaseBremen database = DatabaseBremen.getInstance();
		String re = "";
		Object[][] a = database.execute("SELECT k, property, object, count(k) FROM lgd_map_resource_k_history WHERE " + (search.contains("*") ? "k LIKE '" + search.replaceAll("\\*", "%") + "%'" : "k='" + search + "'") + " GROUP BY k, property, object ORDER BY k");
		...
		for ( int i = 0; i <  a.length; i++ ) {
			re += addkMappingSearch(i, a[i][0].toString(), a[i][1].toString(), a[i][2].toString(), a[i][3].toString(), a[i][7].toString(), a[i][4].toString(), a[i][5].toString(), a[i][6].toString(), search);
		}
	...
	private static String kvMapping(String search) throws Exception {
		...
		Object[][] a;
		if ( search.contains("-") )
			a = database.execute("SELECT k, v, property, object, count(k) FROM lgd_map_resource_kv_history WHERE k='" + search.split("-")[0] + "' AND v='" + search.split("-")[1] + "' GROUP BY k, v, property, object ORDER BY k, v");
		else
			a = database.execute("SELECT k, v, property, object, count(k) FROM lgd_map_resource_kv_history WHERE " + (search.contains("*") ? "k LIKE '" + search.replaceAll("\\*", "%") + "%'" : "k='" + search + "'") + " OR " + (search.contains("*") ? "v LIKE '" + search.replaceAll("\\*", "%") + "%'" : "v='" + search + "'") + " GROUP BY k, v, property, object ORDER BY k, v");
		...
		re += "\t\t\t\t<table class=\"table\">\n";
		re += "\t\t\t\t\t<tr class=mapping>\n";
		re += "\t\t\t\t\t\t<th><a href=\"?tab=history" + (search.equals("") ? "" : "&search=" + search) + "\">timestamp</a></th>\n";
		re += "\t\t\t\t\t\t<th><a href=\"?tab=history" + (search.equals("") ? "" : "&search=" + search) + "\">k</a></th>\n";
		re += "\t\t\t\t\t\t<th><a href=\"?tab=history" + (search.equals("") ? "" : "&search=" + search) + "\">v</a></th>\n";
		re += "\t\t\t\t\t\t<th>property</th>\n";
		re += "\t\t\t\t\t\t<th>object</th>\n";
		re += "\t\t\t\t\t\t<th>affected Entities</th>\n";
		re += "\t\t\t\t\t\t<th>action</th>\n";
		re += "\t\t\t\t\t\t<th><a href=\"?tab=history" + (search.equals("") ? "" : "&search=" + search) + "\">user</a></th>\n";
		re += "\t\t\t\t\t\t<th>comment</th>\n";
		re += "\t\t\t\t\t\t<th>restore</th>\n";
		re += "\t\t\t\t\t</tr>\n";

		for ( int i = 0; i <  a.length; i++ ) {
			re += addkvMappingSearch(i, a[i][0].toString(), a[i][1].toString(), a[i][2].toString(), a[i][3].toString(), a[i][4].toString(), a[i][8].toString(), a[i][5].toString(), a[i][6].toString(), a[i][7].toString(), search);
		}
	...
	private static String datatypeMapping(String search) throws Exception {
		...
		if ( search.contains("-") )
			a = database.execute("SELECT k, datatype, count(k) FROM lgd_map_datatype_history WHERE k='" + search.split("-")[0] + "' " + (search.split("-")[1].equals("int") || search.split("-")[1].equals("float") || search.split("-")[1].equals("boolean") ? "AND datatype='" + search.split("-")[1] + "'" : "" ) + " GROUP BY k, datatype ORDER BY k, datatype");
		else
			a = database.execute("SELECT k, datatype, count(k) FROM lgd_map_datatype_history WHERE " + (search.contains("*") ? "k LIKE '" + search.replaceAll("\\*", "%") + "%'" : "k='" + search + "'") + " GROUP BY k, datatype ORDER BY k, datatype");
		...
		for ( int i = 0; i <  a.length; i++ ) {
			re += addDatatypeMappingSearch(i, a[i][0].toString(), a[i][1].toString(), a[i][2].toString(), a[i][6].toString(), a[i][3].toString(), a[i][4].toString(), a[i][5].toString(), search);
		}
	 ...
	 * @return Returns a String with HTML-code.
	 */
	static private String addkMappingSearch(int id, String k, String property, String object, String affectedEntities,String action, String user_id, String comment, String timestamp, String search) {
		String s = "\t\t\t\t\t<tr id=\"k" + id + "a\">\n";
                s += "\t\t\t\t\t\t<td style=\"text-align: right;\">" + Functions.showTimestamp(timestamp) + "</td>\n";
                s += "\t\t\t\t\t\t<td>" + k + "</td>\n";
                s += "\t\t\t\t\t\t<td>" + Functions.shortenURL(property) + "</td>\n";
                ...
                s += "\t\t\t\t\t\t<td><a onclick=\"toggle_visibility('k" + id + "')\">restore</a></td>\n";
                s += "\t\t\t\t\t</tr>\n";
                s += "\t\t\t\t\t<form action=\"?tab=history" + (search.equals("") ? "" : "&search=" + search) + (!User.getInstance().isLoggedIn() ? "&captcha=yes" : "") + "\" method=\"post\" accept-charset=\"UTF-8\" autocomplete=\"off\">\n";  
                s += "\t\t\t\t\t\t<tr id=\"k" + id + "\" class=\"mapping\" style=\"display: none;\">\n";
                s += "\t\t\t\t\t\t<td style=\"text-align: right;\">" + Functions.showTimestamp(timestamp) + "</td>\n";
                s += "\t\t\t\t\t\t\t<td>" + k + "</td>\n";
                s += "\t\t\t\t\t\t\t<td>" + property + "</td>\n";
	 ...
	 * @return Returns a String with HTML-code.
	 */
	static private String addkvMappingSearch(int id, String k, String v, String property, String object, String affectedEntities, String action, String user_id, String comment, String timestamp, String search) {
                String s = "\t\t\t\t\t<tr id=\"kv" + id + "a\">\n";
                s += "\t\t\t\t\t\t<td style=\"text-align: right;\">" + Functions.showTimestamp(timestamp) + "</td>\n";
                s += "\t\t\t\t\t\t<td>" + k + "</td>\n";
                s += "\t\t\t\t\t\t<td>" + v + "</td>\n";
         ...
         * @return Returns a String with HTML-code.
         */
	static private String addDatatypeMappingSearch(int id, String k, String datatype, String affectedEntities, String action, String user_id, String comment, String timestamp, String search) {
                String s = "\t\t\t\t\t<tr id=\"tk" + id + "a\">\n";
                s += "\t\t\t\t\t\t<td style=\"text-align: right;\">" + Functions.showTimestamp(timestamp) + "</td>\n";
                s += "\t\t\t\t\t\t<td>" + k + "</td>\n";
                s += "\t\t\t\t\t\t<td>" + datatype + "</td>\n";
