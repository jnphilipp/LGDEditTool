...
try {
	...
	Object[][] a = database.execute("SELECT k, v, count FROM " + User.getInstance().getView() + "_unmapped WHERE UPPER(k) LIKE UPPER('" + query + "') OR UPPER(v) LIKE UPPER('" + query + "') OR (k, v) IN (SELECT k, v FROM lgd_map_label WHERE UPPER(label) LIKE UPPER('" + query + "'))");
